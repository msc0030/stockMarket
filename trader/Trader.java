...
public class Trader {
	...
	public void buyFromBank(Market market, String symbol, int volume)
		...
		if (market.getStockForSymbol(symbol) == null) {
			throw new StockMarketException("Stock not present (" + symbol + ")");
		}

		double stockPrice = market.getStockForSymbol(symbol).getPrice();
	...
	public void placeNewOrder(Market market, String symbol, int volume,
		...
		switch (orderType){
		case BUY:
                  	placeNewBuyOrder(market, symbol, volume, price);
			break;
		case SELL:
                  	placeNewSellOrder(market, symbol, volume, price);
			break;
		}
	}
